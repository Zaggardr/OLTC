...
def build_normalized_multicapteur(df: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    palette = [ACCENT_BRIGHT, C_CRIT, C_INFO, C_WARN, "#A78BFA"]
    sample = df.iloc[:: max(1, len(df) // 500)]
    for i, param in enumerate(api.NOMINAL.keys()):
        nom = api.NOMINAL[param]["mean"]
...
def render_kpi_row(hi_now, status_label, if_score_now, last_fault_key, last_fault_date, mwh_avoided, hi_delta):
    ...
    if_tone = "ok" if if_score_now < 0.47 else ("warn" if if_score_now < 0.60 else "crit")
    if_value_tone = if_tone

    st_tone = status_tone(status_label)
    ...
    with k2:
        st.markdown(tile_html(
            "Statut Global", status_label.upper(),
            delta="État courant du système",
            tone=st_tone, value_tone=st_tone, icon="●"
        ), unsafe_allow_html=True)
    with k3:
        if_label = "Normal" if if_score_now < 0.47 else ("Alerte" if if_score_now < 0.60 else "Critique")
        st.markdown(tile_html(
            "Score Anomalie IF", f"{if_score_now:.3f}",
            delta=f"Seuil 0.60 → {if_label}",
            tone=if_tone, value_tone=if_value_tone, icon="◈"
        ), unsafe_allow_html=True)
    with k4:
        st.markdown(tile_html(
            "Dernière Panne", last_fault_key,
            delta=last_fault_date,
            tone="info", icon="⚠"
        ), unsafe_allow_html=True)
    with k5:
        st.markdown(tile_html(
            "MWh Évités", f"{mwh_avoided:.1f}",
            delta="Maintenance prédictive",
            tone="ok", value_tone="ok", icon="⚡"
        ), unsafe_allow_html=True)


def render_sensor_grid(current_vals, spark_data):
    cols = st.columns(5)
    for i, param in enumerate(api.NOMINAL.keys()):
...
def main():
    ...
    def _live_section():
        ...
        render_sensor_grid(current_vals, spark_data)
    ...
    tab_if, tab_multi, tab_dist = st.tabs([
        "Isolation Forest",
    ...
    with tab_multi:
        st.plotly_chart(build_normalized_multicapteur(df), use_container_width=True,
                        config={"displayModeBar": False})
